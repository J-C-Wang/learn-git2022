        ...
        <!-- * 语义化
          ...
          * 属性
              ...
              * 如下列出一些，通用/全局属性(Global Attributes)，即可以用在所有元素上面且有实际意义
                  ...
                  + style属性，用于给标签指定内联样式
                  + <input type="text"> 添加可输入的文本框
                  + tabindex="数字" tab键切换文本框顺序
                  + contenteditable(5) 内容可编辑，加入代码里面就可以
                  + data-* 给标签加属性（不建议使用）
          ...
          * 以下介绍各个标签
            ...
            * head
                ...
                * 头部标签，放一些与页面相关的“元信息”，比如：
                    * 页面的编码方式
                        - `<meta charset="utf-8">`（只有这个可简化）
                            + chartset：字符集
                            + meta：元
                        - <meta name="description" content="铭师堂教育，让中国人都有好书读"> （非简化展开代码）
                        * 编码方式又是一个很大的话题
                        * 一般的乱码都是因为编码方式引起的
                    * 页面的标题，即title标签
